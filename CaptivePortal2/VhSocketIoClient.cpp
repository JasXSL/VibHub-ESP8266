/*
    ...
    Contains specifically needed modifications for Vh
    Main changes:
        ESP32 compatibility
        Fire "disconnect" for connection lost as well (may fire more than once as a result)
*/
...
void SocketIoClient::webSocketEvent(WStype_t type, uint8_t * payload, size_t length) {
	...
	switch(type) {
		case WStype_DISCONNECTED:
            ...
            trigger("disconnect", NULL, 0); //KC: also dire disconnect when connection to server is lost
			break;
		...
		case WStype_BIN:
			...
			SOCKETIOCLIENT_DEBUG("0x%", payload);
		break;
	}
}
